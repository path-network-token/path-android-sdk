...
static int tcp_init(const sockaddr_any *dest,
    ...
    */
    RET_ERROR_NEGATIVE(icmp_sk = socket(af, SOCK_RAW, (af == AF_INET) ? IPPROTO_ICMP : IPPROTO_ICMPV6), "socket");

    /*  icmp_sk not need full tune_socket() here, just a receiving one  */
    RET_NON_ZERO(bind_socket(icmp_sk));

    use_timestamp(icmp_sk);
    use_recv_ttl(icmp_sk);

    return add_poll(icmp_sk, POLLIN);
}
...
static int tcp_send_probe(probe *pb, int ttl) {
    ...
    RET_ERROR_NEGATIVE(getsockname(sk, &addr.sa, &length), "getsockname");

    pb->seq = addr.sin.sin_port;    /*  both ipv4/ipv6  */

    pb->sk = sk;

    return add_poll(sk, POLLERR | POLLHUP | POLLOUT);
}


static probe *tcp_check_reply(probe *probes, unsigned int num_probes, int sk, int err, sockaddr_any *from,
                              char *buf, size_t len) {
    int af = dest_addr.sa.sa_family;
    int type, code, info;
    probe *pb;
    ...
    pb = probe_by_seq(probes, num_probes, tcp->source);
    if (!pb) return NULL;
...
static int tcp_recv_probe(probe *probes, unsigned int num_probes, int sk, int revents) {
    ...
    return recv_reply(probes, num_probes, icmp_sk, 0, tcp_check_reply);
}
