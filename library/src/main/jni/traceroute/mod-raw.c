...
static probe *raw_check_reply(probe *probes, unsigned int num_probes, int sk, int err, sockaddr_any *from,
                              char *buf, size_t len) {
    probe *pb;
    ...
    pb = probe_by_seq(probes, num_probes, seq);
    if (!pb) return NULL;
...
static int raw_recv_probe(probe *probes, unsigned int num_probes, int sk, int revents) {

    if (!(revents & (POLLIN | POLLERR)))
        return 0;

    return recv_reply(probes, num_probes, sk, !!(revents & POLLERR), raw_check_reply);
}
