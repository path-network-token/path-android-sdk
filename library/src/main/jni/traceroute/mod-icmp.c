...
static int icmp_init(const sockaddr_any *dest,
    ...
    RET_NON_ZERO(tune_socket(icmp_sk));

    /*  Don't want to catch packets from another hosts   */
    if (raw_can_connect()) {
        RET_ERROR_NEGATIVE(connect(icmp_sk, &dest_addr.sa, sizeof(dest_addr)), "connect");
    }

    RET_NON_ZERO(use_recverr(icmp_sk));
    ...
    if (dgram) {
        ...
        RET_ERROR_NEGATIVE(getsockname(icmp_sk, &addr.sa, &len), "getsockname");
    ...
    return add_poll(icmp_sk, POLLIN | POLLERR);
}
...
static int icmp_send_probe(probe *pb, int ttl) {
    ...
    if (ttl != last_ttl) {
        ...
        RET_NON_ZERO(set_ttl(icmp_sk, ttl));
    ...
    int ret = do_send(icmp_sk, data, *length_p, &dest_addr);
    if (ret < 0) {
        pb->send_time = 0;
        return ret;
    }
...
static probe *icmp_check_reply(probe *probes, unsigned int num_probes, int sk, int err, sockaddr_any *from,
                               char *buf, size_t len) {
    int af = dest_addr.sa.sa_family;
    int type;
    ...
    pb = probe_by_seq(probes, num_probes, recv_seq);
    if (!pb) return NULL;
...
static int icmp_recv_probe(probe *probes, unsigned int num_probes, int sk, int revents) {

    if (!(revents & (POLLIN | POLLERR)))
        return 0;

    return recv_reply(probes, num_probes, sk, !!(revents & POLLERR), icmp_check_reply);
}
