...
int error(const char *str) {
    __android_log_print(ANDROID_LOG_ERROR, LOG_TAG, "%s [%d]: %s", str, errno, strerror(errno));
    return -1;
}
...
static int make_fd_used(int fd) {
    int nfd;

    if (fcntl(fd, F_GETFL) != -1) return 0;

    if (errno != EBADF) return error("fcntl F_GETFL");

    RET_ERROR_NEGATIVE(nfd = open("/dev/null", O_RDONLY), "open /dev/null");
...
static int init_ip_options(void) {
    ...
    if (num_gateways > max)
        return ex_error("Too many gateways specified. No more than %d", max);
    ...
    for (i = 0; i < num_gateways; i++) {
        ...
        if (!gateways[i]) return error("strdup");

        if (getaddr(gateways[i], &gates[i]) < 0)
            return ex_error("");    /*  already reported   */
        if (gates[i].sa.sa_family != af)
            return ex_error("IP versions mismatch in gateway addresses");
...
static int add_gateway(CLIF_option *optn, char *arg) {
    ...
    if (num_gateways >= MAX_GATEWAYS_6) {    /*  127 > 8 ... :)   */
        __android_log_print(ANDROID_LOG_ERROR, LOG_TAG, "Too many gateways specified.");
        return -1;
    }

    gateways = realloc(gateways, (num_gateways + 1) * sizeof(*gateways));
    if (!gateways) return error("malloc");
...
static int set_mod_option(CLIF_option *optn, char *arg) {
    ...
    if (!strcmp(arg, "help")) {
        ...
        if (mod && mod->options) {
            ...
            __android_log_print(ANDROID_LOG_ERROR, LOG_TAG, "No options for module `%s'\n", module);

        return 0;
    }

    if (opts_idx >= sizeof(opts) / sizeof(*opts)) {
        __android_log_print(ANDROID_LOG_ERROR, LOG_TAG, "Too many module options\n");
        return -1;
    }

    opts[opts_idx] = strdup(arg);
    if (!opts[opts_idx]) return error("strdup");
    opts_idx++;
...
int traceroute(int argc, char *argv[], int *count, probe_result **probe_results) {
    ...
    if (!ops) return ex_error("Unknown traceroute module %s", module);


    if (!first_hop || first_hop > max_hops)
        return ex_error("first hop out of range");
    if (max_hops > MAX_HOPS)
        return ex_error("max hops cannot be more than " _TEXT(MAX_HOPS));
    if (!probes_per_hop || probes_per_hop > MAX_PROBES)
        return ex_error("no more than " _TEXT(MAX_PROBES) " probes per hop");
    if (wait_secs < 0)
        return ex_error("bad wait seconds `%g' specified", wait_secs);
    if (packet_len > MAX_PACKET_LEN)
        return ex_error("too big packetlen %d specified", packet_len);
    if (src_addr.sa.sa_family && src_addr.sa.sa_family != af)
        return ex_error("IP version mismatch in addresses specified");
    if (send_secs < 0)
        return ex_error("bad sendtime `%g' specified", send_secs);
    ...
    RET_NON_ZERO(make_fd_used(0));
    RET_NON_ZERO(make_fd_used(1));
    RET_NON_ZERO(make_fd_used(2));


    RET_NON_ZERO(init_ip_options());
...
static void print_header(void) {
    ...
    __android_log_print(ANDROID_LOG_INFO, LOG_TAG, "traceroute to %s (%s), %u hops max, %zu byte packets",
                        dst_name, addr2str(&dst_addr), max_hops,
                        header_len + data_len);
}


static void print_addr(sockaddr_any *res, probe_result *result, bool useLog) {
    const char *str;
    ...
    // Copy IP
    snprintf(result->ip, sizeof(result->ip) - 1, "%s", str);

    if (noresolve) {
        if (useLog) __android_log_print(ANDROID_LOG_INFO, LOG_TAG, " %s", str);
    } else {
        char buf[1024];

        buf[0] = '\0';
        getnameinfo(&res->sa, sizeof(*res), buf, sizeof(buf), 0, 0, 0); //NI_IDN
        if (useLog) __android_log_print(ANDROID_LOG_INFO, LOG_TAG, " %s (%s)", buf[0] ? buf : str, str);

        // Copy host
        snprintf(result->host, sizeof(result->host) - 1, "%s", buf[0] ? buf : str);
    }

    if (as_lookups) {
        const char *path = get_as_path(str);
        if (path) {
            if (useLog) __android_log_print(ANDROID_LOG_INFO, LOG_TAG, " [%s]", path);
        }
    }
}
...
static void print_probe(probe *pb, probe_result *result) {
    ...
    if (!pb->res.sa.sa_family) {
        ...
        if (pb->ext) {
            snprintf(result->ext, sizeof(result->ext) - 1, "%s", pb->ext);
        }
        if (backward && pb->recv_ttl) {
            int hops = ttl2hops(pb->recv_ttl);
            if (hops != ttl) {
                result->recv_ttl = hops;
            }
        }
        ...
        print_addr(&pb->res, result, prn);
        if (prn) {
            if (pb->ext) __android_log_print(ANDROID_LOG_INFO, LOG_TAG, " <%s>", pb->ext);

            if (backward && pb->recv_ttl) {
                int hops = ttl2hops(pb->recv_ttl);
                if (hops != ttl) __android_log_print(ANDROID_LOG_INFO, LOG_TAG, " '-%d'", hops);
            }
        }
    }
    ...
    if (pb->recv_time) {
        ...
        __android_log_print(ANDROID_LOG_INFO, LOG_TAG, "  %.3f ms", diff * 1000);

        result->delay = diff;
    }

    if (pb->err_str[0]) {
        __android_log_print(ANDROID_LOG_INFO, LOG_TAG, " %s", pb->err_str);
        snprintf(result->err, sizeof(result->err) - 1, "%s", pb->err_str);
    }
    ...
    while (start < end) {
        ...
        for (n = start; n < end; n++) {
            ...
            if (!pb->send_time) {
                ...
                RET_NON_ZERO(ops->send_probe(pb, ttl));

                if (!pb->send_time) {
                    if (max_time) break;    /*  have chances later   */
                    else return error("send probe");
                }
...
int tune_socket(int sk) {
    ...
    if (debug) {
        i = 1;
        RET_ERROR_NEGATIVE(setsockopt(sk, SOL_SOCKET, SO_DEBUG, &i, sizeof(i)), "setsockopt SO_DEBUG");
    }


#ifdef SO_MARK
    if (fwmark) {
        RET_ERROR_NEGATIVE(setsockopt(sk, SOL_SOCKET, SO_MARK, &fwmark, sizeof(fwmark)), "setsockopt SO_MARK");
    }
#endif
    ...
    if (rtbuf && rtbuf_len) {
        if (af == AF_INET) {
            RET_ERROR_NEGATIVE(setsockopt(sk, IPPROTO_IP, IP_OPTIONS, rtbuf, rtbuf_len), "setsockopt IP_OPTIONS");
        } else if (af == AF_INET6) {
            RET_ERROR_NEGATIVE(setsockopt(sk, IPPROTO_IPV6, IPV6_RTHDR, rtbuf, rtbuf_len), "setsockopt IPV6_RTHDR");
        }
    }
    ...
    if (af == AF_INET) {
        ...
        if (setsockopt(sk, SOL_IP, IP_MTU_DISCOVER, &i, sizeof(i)) < 0 &&
            ...
            return error("setsockopt IP_MTU_DISCOVER");

        if (tos) {
            i = tos;
            RET_ERROR_NEGATIVE(setsockopt(sk, SOL_IP, IP_TOS, &i, sizeof(i)), "setsockopt IP_TOS");
        }
        ...
        if (setsockopt(sk, SOL_IPV6, IPV6_MTU_DISCOVER, &i, sizeof(i)) < 0 &&
            ...
            return error("setsockopt IPV6_MTU_DISCOVER");
        ...
        if (flow_label) {
            ...
            RET_ERROR_NEGATIVE(setsockopt(sk, IPPROTO_IPV6, IPV6_FLOWLABEL_MGR, &flr, sizeof(flr)),
                               "setsockopt IPV6_FLOWLABEL_MGR");
        }

        if (tos) {
            i = tos;
            RET_ERROR_NEGATIVE(setsockopt(sk, IPPROTO_IPV6, IPV6_TCLASS, &i, sizeof(i)), "setsockopt IPV6_TCLASS");
        }

        if (tos || flow_label) {
            i = 1;
            RET_ERROR_NEGATIVE(setsockopt(sk, IPPROTO_IPV6, IPV6_FLOWINFO_SEND, &i, sizeof(i)),
                               "setsockopt IPV6_FLOWINFO_SEND");
        }
    }
    ...
    if (noroute) {
        i = noroute;
        RET_ERROR_NEGATIVE(setsockopt(sk, SOL_SOCKET, SO_DONTROUTE, &i, sizeof(i)), "setsockopt SO_DONTROUTE");
    }
...
static int parse_local_res(probe *pb, int ee_errno, int info) {
...
int recv_reply(int sk, int err, check_reply_t check_reply) {
    ...
    */
    ...
    if (ee && ee->ee_origin == SO_EE_ORIGIN_LOCAL) RET_NON_ZERO(parse_local_res(pb, ee->ee_errno, ee->ee_info));
...
int bind_socket(int sk) {
    ...
    if (device) {
        RET_ERROR_NEGATIVE(setsockopt(sk, SOL_SOCKET, SO_BINDTODEVICE, device, strlen(device) + 1),
    ...
    RET_ERROR_NEGATIVE(bind(sk, &addr->sa, sizeof(*addr)), "bind");
...
int use_recverr(int sk) {
    ...
    if (af == AF_INET) {
        RET_ERROR_NEGATIVE(setsockopt(sk, SOL_IP, IP_RECVERR, &val, sizeof(val)), "setsockopt IP_RECVERR");
    } else if (af == AF_INET6) {
        RET_ERROR_NEGATIVE(setsockopt(sk, SOL_IPV6, IPV6_RECVERR, &val, sizeof(val)), "setsockopt IPV6_RECVERR");
    }
    return 0;
}
...
int set_ttl(int sk, int ttl) {
    ...
    if (af == AF_INET) {
        RET_ERROR_NEGATIVE(setsockopt(sk, SOL_IP, IP_TTL, &ttl, sizeof(ttl)), "setsockopt IP_TTL");
    } else if (af == AF_INET6) {
        RET_ERROR_NEGATIVE(setsockopt(sk, SOL_IPV6, IPV6_UNICAST_HOPS, &ttl, sizeof(ttl)),
...
int do_send(int sk, const void *data, size_t len, const sockaddr_any *addr) {
    ...
    if (res < 0) {
        ...
        return error("send");    /*  not recoverable   */
    }
